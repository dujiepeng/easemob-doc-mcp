...
### 变更 (Changes)
- **环境优化**: 手动彻底清理了系统中残留的 `cloudflared` 容器，确保运行环境纯净。

## [1.1.21] - 2025-12-23

### 变更 (Changes)
- **网络优化**: 将 `docker_default` 网络配置改为自动创建模式（由 `external` 改为 `bridge` 驱动），并保障了容器的互联网访问连通性。

## [1.1.20] - 2025-12-23

### 变更 (Changes)
- **精简服务**: 移除了所有老旧路径（如 `/` 和 `/mcp/`）的兼容性重定向，仅保留官方 `/sse` 端点。

## [1.1.19] - 2025-12-23

### 修复 (Bug Fixes)
- **兼容性修复**: 添加了 `/mcp/` 路径到 `/sse` 的自动重定向，解决了维护工具或旧配置访问导致的 404 问题。
...
### 变更 (Changes)
- **网络优化**: 配置服务接入外部网络 `docker_default`，以支持跨容器通信。

## [1.1.16] - 2025-12-23

### 修复 (Bug Fixes)
- **网络修复**: 恢复 Docker `bridge` 模式以兼容 macOS 环境下的端口映射，从而解决容器化后 9000 端口无法访问的问题。
- **启动日志**: 更新启动日志中的版本号显示。

## [1.1.15] - 2025-12-23

### 变更 (Changes)
- **网络优化**: 将 Docker 网络模式更改为 `host`，确保容器内部可以完全访问宿主机及外网。
